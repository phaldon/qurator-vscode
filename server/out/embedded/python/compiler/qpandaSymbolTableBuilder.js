...
(function (PyQpandaSymbolTableBuilder) {
    ...
    function loadPyQpandaSymbolsAt(scope) {
        pyQpandaSDK_1.PyQpandaSDK.classesAndMethods().forEach(qclassAndMethods => {
            let type = scope.lookup(symbols_1.PyQpandaSymbols.class);
            let args = getArgumentsSymbols(qclassAndMethods.arguments, scope);
            let methods = getMethodsSymbols(qclassAndMethods.methods, scope);
            let classSymbol = new symbols_1.ClassSymbol(qclassAndMethods.name, type, args, methods);
